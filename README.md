...
1.  **Start the Flask Backend**:
    Open a new terminal or command prompt, navigate to your project directory, and run your Flask app, where `@app.route` decorators are used):
    ```bash
      python Rag.py
    ```

2.  **Start the Streamlit Frontend**:
    Open another new terminal or command prompt, navigate to your project directory, and run your Streamlit app. 
    ```bash
...
*   **"ERROR: Docker Not Running" or Qdrant Connectivity Issues**:
    *   **Solution**: Ensure Docker Desktop is installed, running, and the Qdrant container is started via `docker run -d -p 7000:6333 -v "C:\Users\bruce\OneDrive\Desktop\New folder\PYTHON\Demo\Endpoint_Files\qdrant_data:/qdrant/storage" --name qdrant-rag qdrant/qdrant`. Check your firewall settings if issues persist.
